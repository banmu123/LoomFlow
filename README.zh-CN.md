...
- **回归检测**：自动基线对比（版本/生产/滚动），覆盖 5 个指标（成功率、失败率、P95 延迟、成本、测试得分），支持相对 + 绝对阈值
- **AI 分析**：指标聚合 → 瓶颈检测 → 静态分析 → AI 生成优化 Patch
- **人工确认**：提案需用户明确确认后才会修改生产工作流
- **演化历史**：完整可追溯——触发 → 分析 → 提案 → 决策 → 版本变更 → 效果（前后指标对比）
...
│   ├── workflow-eval/    # 工作流评估（指标、瓶颈、静态分析、AI 优化、回归检测、基线管理）
│   ├── workflow-copilot/ # Copilot 管线（patch、proposal、diff、测试用例）
│   ├── evolution/        # 演化引擎（规则评估、触发检测、编排、调度、回归事件）
│   ├── evolution-history/# 演化历史（时间线、效果、聚合、查询）
...
- **Vitest** — 735 个单元测试（引擎、schema、沙箱、执行器、搜索适配层、密钥加密、执行追踪、笔记、模型注册表、节点注册表、Agent 工具、i18n、演化引擎、回归检测、演化历史、质量门禁）
