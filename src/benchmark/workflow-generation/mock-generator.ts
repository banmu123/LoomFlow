...
function generateMockWorkflow(input: string): TinyflowData {
  ...
  if (inputLower.includes('ai') || inputLower.includes('对话') || inputLower.includes('聊天')) {
    return {
      nodes: [
        ...
        { id: 'llm', type: 'llmNode', position: { x: 200, y: 0 }, data: { title: 'AI处理', llmId: 'deepseek-v4-flash' } },
  ...
  if (inputLower.includes('api') || inputLower.includes('http') || inputLower.includes('天气')) {
    return {
      nodes: [
        { id: 'start', type: 'startNode', position: { x: 0, y: 0 }, data: { title: '开始' } },
        { id: 'http', type: 'httpNode', position: { x: 200, y: 0 }, data: { title: '调用API', method: 'GET', url: 'https://api.example.com' } },
        { id: 'end', type: 'endNode', position: { x: 400, y: 0 }, data: { title: '结束' } },
      ],
      edges: [
  ...
  if (inputLower.includes('搜索') || inputLower.includes('研究') || inputLower.includes('报告')) {
    return {
      nodes: [
        { id: 'start', type: 'startNode', position: { x: 0, y: 0 }, data: { title: '开始' } },
        { id: 'search', type: 'searchEngineNode', position: { x: 200, y: 0 }, data: { title: '搜索资料' } },
        { id: 'llm', type: 'llmNode', position: { x: 400, y: 0 }, data: { title: '生成报告', llmId: 'deepseek-v4-flash' } },
        { id: 'template', type: 'templateNode', position: { x: 600, y: 0 }, data: { title: '格式化输出' } },
        { id: 'end', type: 'endNode', position: { x: 800, y: 0 }, data: { title: '结束' } },
      ],
      edges: [
  ...
  if (inputLower.includes('客服') || inputLower.includes('分类') || inputLower.includes('支持')) {
    return {
      nodes: [
        { id: 'start', type: 'startNode', position: { x: 0, y: 0 }, data: { title: '开始' } },
        { id: 'classify', type: 'llmNode', position: { x: 200, y: 0 }, data: { title: '问题分类', llmId: 'deepseek-v4-flash' } },
        { id: 'condition', type: 'conditionNode', position: { x: 400, y: 0 }, data: { title: '判断类型' } },
        { id: 'answer', type: 'llmNode', position: { x: 600, y: -50 }, data: { title: '生成回答', llmId: 'deepseek-v4-flash' } },
        { id: 'transfer', type: 'templateNode', position: { x: 600, y: 50 }, data: { title: '转人工' } },
        { id: 'end', type: 'endNode', position: { x: 800, y: 0 }, data: { title: '结束' } },
      ],
      edges: [
  ...
  if (inputLower.includes('文章') || inputLower.includes('内容') || inputLower.includes('生成')) {
    return {
      nodes: [
        { id: 'start', type: 'startNode', position: { x: 0, y: 0 }, data: { title: '开始' } },
        { id: 'outline', type: 'llmNode', position: { x: 200, y: 0 }, data: { title: '生成大纲', llmId: 'deepseek-v4-flash' } },
        { id: 'content', type: 'llmNode', position: { x: 400, y: 0 }, data: { title: '生成内容', llmId: 'deepseek-v4-flash' } },
        { id: 'format', type: 'templateNode', position: { x: 600, y: 0 }, data: { title: '格式化' } },
        { id: 'end', type: 'endNode', position: { x: 800, y: 0 }, data: { title: '结束' } },
      ],
      edges: [
  ...
  if (inputLower.includes('excel') || inputLower.includes('数据') || inputLower.includes('批量')) {
    return {
      nodes: [
        { id: 'start', type: 'startNode', position: { x: 0, y: 0 }, data: { title: '开始' } },
        { id: 'loop', type: 'loopNode', position: { x: 200, y: 0 }, data: { title: '遍历数据' } },
        { id: 'process', type: 'codeNode', position: { x: 400, y: 0 }, data: { title: '处理数据' } },
        { id: 'llm', type: 'llmNode', position: { x: 600, y: 0 }, data: { title: '生成分析', llmId: 'deepseek-v4-flash' } },
        { id: 'excel', type: 'excelNode', position: { x: 800, y: 0 }, data: { title: '导出Excel' } },
        { id: 'end', type: 'endNode', position: { x: 1000, y: 0 }, data: { title: '结束' } },
      ],
      edges: [
  ...
  return {
    nodes: [
      { id: 'start', type: 'startNode', position: { x: 0, y: 0 }, data: { title: '开始' } },
      { id: 'llm', type: 'llmNode', position: { x: 200, y: 0 }, data: { title: '处理', llmId: 'deepseek-v4-flash' } },
      { id: 'end', type: 'endNode', position: { x: 400, y: 0 }, data: { title: '结束' } },
    ],
    edges: [
      { id: 'e1', source: 'start', target: 'llm' },
